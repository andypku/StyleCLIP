...
def main(args):
    ...
    text_inputs = torch.cat([clip.tokenize(args.description)]).cpu()
    ...
    if args.latent_path:
        latent_code_init = torch.load(args.latent_path).cpu()
    elif args.mode == "edit":
        latent_code_init_not_trunc = torch.randn(1, 512).cpu()
        with torch.no_grad():
